...
## ASSET:ARCHITECTURE 2026-08-17 06:35 ▸ Typography migrated to Fraunces + DM Sans (Google Fonts, `<link>`-loaded); homepage redesigned with avocado hero art, horizontal screenshot gallery, and a floating pill navbar; support inbox standardized to `success@toifood.co.nz`

Five commits since the 2026-08-10 06:54 entry (still current for the recipe-migration/marketing-only architecture — no route or Pages Function changes this period): HEAD moved `0abd9e9` → `626e222` (2026-08-11 to 2026-08-15).

1. **Font system replaced.** `--font-display` moved from a self-hosted `Americana.otf` `@font-face` to `'Fraunces', serif`; `--font-body` moved from `system-ui` to `'DM Sans', system-ui, sans-serif` (`b5c4346`). Initially loaded via a CSS `@import` in `global.css` (render-blocking, discovered late in the cascade); corrected the same day (`617ccb2`) to `<link rel="preconnect">` + `<link rel="stylesheet">` in `frontend/index.html`, the standard non-blocking pattern, with the CSS `@import` removed.
2. **Homepage redesign** (`73ea794`, `626e222`): new hero character/avocado artwork (`hero-char.png`, `hero-char2.png`, `hero-char3.png`, `frontend/public/features/avocado_*_cutout.png`) replaces the prior `PhoneMockup` CSS mockup on the hero; a new horizontally-scrolling screenshot gallery section (`frontend/public/promo/1-7.png`, 7 rotated/staggered device-frame cards) was inserted between the hero and the features grid. `Navbar.jsx` changed from a full-width fixed bar to a floating rounded-pill nav (`position: absolute` on desktop / `fixed` on mobile ≤640px, `border-radius: 20px`, drop shadow), with its own mobile-resize listener; the nav's "Download" link now points at `/` and smooth-scrolls to the hero's `#download` anchor instead of assuming a lower on-page anchor. `body { padding-top }` dropped to 0 since the navbar is no longer a fixed full-width element pushing content down.
3. **Support email standardized**: `support@toifood.co.nz` → `success@toifood.co.nz` across `FAQ.jsx` (2 refs) `Privacy.jsx` (2 refs), and `Terms.jsx` (1 ref), matching the address `Contact.jsx` already used since its 2026-08-03 addition.
4. **Known current-state defect** (see ISSUE log): `--primary` currently resolves to the same value as `--bg` (`#F5EFE7`) rather than the brand lime (`#96cf24`), live on `main` as of this HEAD — treat the accent color as broken, not as a deliberate rebrand, until resolved.
5. **Recovery posture unchanged**: still a single Cloudflare Pages redeploy (`vite build` → `dist`) for `toifood-web` plus an independent `wrangler deploy` for the untouched `toifood-og` worker; no database, Prisma, or server state in either service.
