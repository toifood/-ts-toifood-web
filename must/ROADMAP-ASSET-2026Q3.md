...
- **Sitemap proxy** — `functions/sitemap.xml.js` still fetches and re-serves the backend sitemap (1hr edge cache).
...
- **AI-safety messaging** — `utils/announcementNote.js` still emits WARNING notes for critical allergens and HEADSUP notes for dietary guidelines, unchanged.
- **og-worker** — still present in the tree (`og-worker/src/index.js`, wrangler.toml, package.json) but confirmed via full-tree search to still have no inbound caller anywhere in `frontend/`, consistent with the 2026-08-24 finding (see ROADMAP ISSUE log item 3).
